...
import numpy as np
from kiwiGym_CS2 import kiwiGym
# %%
register(
    id='kiwiGym-CS2_0',                                
    entry_point='KiwiGym_env_CS2_0:kiwiGymEnv_CS2_0', 
)


class kiwiGymEnv_CS2_0(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        TH_param_mean=np.array([1.2578, 0.43041, 0.6439,  7.0767,  0.4063,  0.1143*4,  0.1848*4,    .4242,    1.586*.7, 1.5874*.7,  0.3322*.75,  0.0371,  0.0818,    9000, .1, 5]+[850]*3+[90]*3)
        ...
        time_step_before=1
        obs[0]=int(self.kiwiGym.time_pulses[0]-time_step_before)
...
if __name__=="__main__":
    ...
    while(cnt<3):
        ...
        if(terminated):
            ...
            TH_param=np.array([1.2578, 0.43041, 0.6439,  7.0767,  0.4063,  0.1143*4,  0.1848*4,    .4242,    1.586*.7, 1.5874*.7,  0.3322*.75,  0.0371,  0.0818,    9000, .1, 5]+[850]*3+[90]*3)
            obs = env.reset()[0]
        cnt+=1
