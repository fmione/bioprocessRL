...
register(
    id='kiwiGym-v2',                                # call it whatever you want
    entry_point='KiwiGym_createEnv_v2:kiwiGymEnv2', # module_name:class_name
)

# Implement our own gym env, must inherit from gym.Env
# https://gymnasium.farama.org/api/env/
class kiwiGymEnv2(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        e_vector=np.tile([1],self.kiwiGym.time_final)
        d_vector=np.tile([21],self.kiwiGym.time_final*self.kiwiGym.number_mbr)
        ...
        self.observation_space = spaces.Box(
            low=np.zeros(self.kiwiGym.time_final+self.kiwiGym.number_mbr*self.kiwiGym.time_final+29*self.kiwiGym.number_mbr*self.kiwiGym.time_final),
    ...
    def reset(self, seed=None, options=None):
        ...
        TH_reset=TH_param_mean*(1+0.05*(np.random.random(len(TH_param_mean))-.5)/2)

        # Reset the env. 
        self.kiwiGym.reset(seed=seed,TH_param=TH_reset)

        # Construct the observation state:
        obs = np.zeros(len(self.observation_upper_bound))
        self.obs=obs
    ...
    def step(self, action):
        ...
        obs[0:self.kiwiGym.time_final]=0
        obs[self.kiwiGym.time_current-1]=1  
        
        index_action=np.arange(self.kiwiGym.number_mbr)+self.kiwiGym.number_mbr*(self.kiwiGym.time_current-1)+self.kiwiGym.time_final
        
        obs[index_action]=np.array(action)
        index_obs=np.arange(29*self.kiwiGym.number_mbr)+29*self.kiwiGym.number_mbr*(self.kiwiGym.time_current-1)+(1+self.kiwiGym.number_mbr)*(self.kiwiGym.time_final)
        ...
        return obs/self.observation_upper_bound, reward, terminated, False, info
