...
# %%    
    def render(self):
        ...
        for i2 in range(self.uu[0][0]):
            ...
            # plt.plot(self.DD_historic[i2]['Feed_pulse'],'o')
        plt.show()
...
# %%    
    def perform_action(self,action_step=[]):
        ...
        if len(self.obs)==0:
            XX_obs=np.zeros([self.uu[0][0]*(1+1)]) #CHECK
        else:
        ...
        if self.time_current>=self.time_final:
            ...
            for i2 in range(self.uu[0][0]): 
                ...
                # glc_max=max(XX['sample'][i2][1][6:]) #Glucose after batch phase
                # print("Glc",XX['sample'][i2][1])
                # Glc_max.append(glc_max)
                # if glc_max>.5:
                #     glc_constrain=((glc_max-.5)*2+1)**2
                # else:
                #     glc_constrain=1    
                glc_constrain=0
            ...
            print("reward: ",self.reward, "div: ",DIV_min ,"dot: ",min(DOT_min))#, "glucose: ",max(Glc_max)
            # print(Glc_max)
        else:
            self.terminated=False
            self.reward=0
